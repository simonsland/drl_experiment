...
class DeepQNetwork:
    ...
    def learn(self):
        ...
        if self.learn_step_counter % 50 == 0 and hasattr(self, 'merged'):
            loss_log = self.sess.run(self.merged, feed_dict={self.s: batch_memory[:, :self.n_features], self.q_target: q_target})
            self.writer.add_summary(loss_log, self.learn_step_counter)
