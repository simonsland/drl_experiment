...
class OffloadingV2:
    def __init__(self, user_n, task_t, f_mec, f_unit, f_ue, p_ue, bandwidth, w1, w2, reward_function='v_1'):
        ...
        self.request_choice, self.request = self.request_generate()  # 用户请求任务
        self.reward_function = reward_function  # 奖励函数

        # 观测空间: [计算资源分配情况 用户请求]
        self.observation_n = 2 * self.user_n
        self.observation = np.zeros(self.observation_n, dtype=float)  # 观测空间
        self.observation[0:self.user_n] = np.zeros(self.user_n, dtype=float)  # 资源分配
        self.observation[self.user_n:] = self.request_choice  # 用户请求
        ...
        self.consumption_record_local = []  # 本地计算系统消耗变化
        # self.offload_record = []  # 系统卸载决策覆盖人数的变化

    def next_time_slot_init(self):
        # 产生下一个时隙的环境状态
        self.observation[0:self.user_n] = np.zeros(self.user_n, dtype=float)  # 用户资源分配
        self.request_choice, self.request = self.request_generate()  # 用户请求任务
        self.observation[self.user_n:] = self.request_choice
        # self.offload_user = 1

    def reset(self):
        # 重置资源状态
        self.observation[0:self.user_n] = np.zeros(self.user_n, dtype=float)
        self.current_consumption = self.weight_sum()
        self.consumption_record_local.append(self.current_consumption)
    ...
    def step(self, action):
        ...
        while sum(self.observation[0:self.user_n]) <= self.f_mec:
            ...
            if consumption < self.current_consumption:
                ...
                break
            self.observation[action] += self.f_unit
        if sum(self.observation[0:self.user_n]) > self.f_mec:
            done = True
    ...
    def request_generate(self):
        ...
        for choice in choice_arr:
            ...
            user_i += 1
        return choice_arr, request.flatten()
